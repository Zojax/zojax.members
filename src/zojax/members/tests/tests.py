...
zojaxMembers = ZCMLLayer(
    os.path.join(os.path.split(__file__)[0], 'ftesting.zcml'),
    __name__, 'zojaxMembers', allow_teardown=True)
...
def test_suite():
    return unittest.TestSuite((
            FunctionalDocFileSuite("join.txt"),
            FunctionalDocFileSuite("members.txt"),
            FunctionalDocFileSuite("invitation.txt"),
            ))
