...
class MemberInvitationNotification(MailTemplateBase):

    space = None

    def update(self):
        super(MemberInvitationNotification, self).update()

        context = self.context
        request = self.request

        group = removeAllProxies(context.object)
        ...
        try:
            principal = getUtility(
                IAuthentication).getPrincipal(context.principal)
    ...
    @property
    def subject(self):
        return u'%s: %s invites you to join %s'%(
